...
public class TN3270ExtendedCommand extends AbstractExtendedCommand {
    ...
    public byte[] getData() {
        ...
        System.arraycopy(commandHeader.getData(), 0, combinedData, 0, commandHeader.size());
        System.arraycopy(command.getData(), 0, combinedData, commandHeader.size(), command.size());
        return combinedData;
    }
    ...
    public Optional<Buffer> getReply() {
        ...
        if (reply.isPresent()) {
            ...
            TN3270ExtendedCommand command =
                    new TN3270ExtendedCommand(header, (Command) reply.get(), telnetState, charset);
            buffers.add(command);
        }
