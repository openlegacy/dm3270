...
public class BindCommand extends AbstractExtendedCommand {
    ...
    private static final String[] PRESENTATION_TEXT = {
        ...
        "12 x 40",
        "24 x 80",
        "default 24 x 80, alternate in Query Reply",
    ...
    public BindCommand(
            CommandHeader commandHeader, byte[] buffer, int offset, int length, Charset charset) {
    ...
    public String toString() {
        ...
        text.append(String.format("Format ............... %02X  %s%n", format, "must be zero"));
        text.append(
                String.format(
                        "Type ................. %02X  %s%n",
                        type, (type == 1 ? "non-" : "") + "negotiable"));
        text.append("\n");
        ...
        text.append("\n--- Presentation Space --\n");
        text.append(String.format("Query supported ...... %02X  %s%n", flags, querySupported));
        text.append(
                String.format("Presentation space ... %02X  %s%n", presentationSpace, presText));
        text.append(String.format("Default rows ......... %02X  %3d%n", primaryRows, primaryRows));
        text.append(
                String.format(
                        "Default columns ...... %02X  %3d%n", primaryColumns, primaryColumns));
        text.append(
                String.format("Alternate Rows ....... %02X  %3d%n", alternateRows, alternateRows));
        text.append(
                String.format(
                        "Alternate Columns .... %02X  %3d%n", alternateColumns, alternateColumns));
        text.append("\n");
        text.append(
                String.format("Compression .......... %02X  %s%n", compression, compressionText));
        text.append("\n");
        ...
        if (extraBytes > 0) {
            text.append(charset.toHex(data, userDataOffset + userDataLength + 1, extraBytes));
        }
