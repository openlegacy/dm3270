...
public class WriteCommand extends Command {
    ...
    public WriteCommand(byte[] buffer, int offset, int length, Charset charset) {
        ...
        assert buffer[offset] == Command.WRITE_01
                || buffer[offset] == Command.WRITE_F1
        ...
        alternate =
                buffer[offset] == Command.ERASE_WRITE_ALTERNATE_0D
                        || buffer[offset] == Command.ERASE_WRITE_ALTERNATE_7E;
        writeControlCharacter = length > 1 ? new WriteControlCharacter(buffer[offset + 1]) : null;
    ...
    public void process(Screen screen) {
        ...
        if (eraseWrite) {
            ScreenOption requestedScreenOption =
