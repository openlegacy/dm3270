 ...
 *
 * <p>This class provides a facade to ease usage of dm3270 to java clients.
 */
public class TerminalClient {
     ...
     *
     * @param usesExtended3270 set true to support extended protocol, and false if not. By default
     *     is false.
     */
    ...
    private void setFieldText(Field field, String text) {
        ...
        int cursorPosition =
                field.getDisplayLength() > nextPosition
                        ? field.getFirstLocation() + nextPosition
    ...
    public void setTabulatedInput(String text, int offset) throws NoSuchFieldException {
        ...
        if (!getFields().isEmpty()) {
            Field finalField = screen.getFieldManager().getFieldAt(linearPosition).orElse(null);
            if (finalField == null && offset <= 0) {
                throw new NoSuchElementException(
                        "No field found at position (" + row + "," + column + ")");
            }
            if (finalField == null) {
            ...
            if (offset == 0) {
                ...
                throw new NoSuchElementException(
                        "No fields on screen to skip, " + offset + "tab/s");
            }
        }
    }
     ...
     * Send an Action ID.
     *
     * <p>This method is usually used to send Enter after setting text fields, or to send some other
     * keys (like F1).
     *
     ...
     *     is expected, but also parts of the screen which are not meant to be modified or even
     *     visible.
     */
     ...
     *
     * <p>Prefer using resetAlarm so it is properly reset when checking value. Use this operation
     * only if you are implementing some tracing or debugging and don't want to change the alarm
     * flag status.
     */
     ...
     *
     * @return The position of the cursor in the screen (x contains the column and y the row). If
     *     the cursor is not visible then empty value is returned.
     */
