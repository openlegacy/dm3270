...
public class TelnetListener implements BufferListener, TelnetCommandProcessor {
    ...
    public synchronized void listen(
            TelnetSocket.Source source, byte[] buffer, LocalDateTime dateTime, boolean genuine) {
        assert source == this.source
                : "Incorrect source: " + source + ", expecting: " + this.source;
        ...
        switch (dataType) {
            ...
            case BIND_IMAGE:
                BindCommand bindCommand =
                        new BindCommand(
                                currentCommandHeader, data, offset, length, screen.getCharset());
            ...
            case SSCP_LU_DATA:
                ReplyBuffer extCommand =
                        new TN3270ExtendedCommand(
                                currentCommandHeader,
                                new SscpLuDataCommand(data, offset, length, screen.getCharset()),
                                telnetState,
                                charset);
            ...
            case BID:
                addDataRecord(currentCommandHeader, SessionRecordType.TN3270E);
                break;

            default:
                LOG.warn(
                        "Data type not written: {}, {}",
                        dataType,
                        charset.toHex(data, offset, length));
        }
    }
    ...
    private void addDataRecord(
            ReplyBuffer message, SessionRecord.SessionRecordType sessionRecordType) {
        // no gui involved
