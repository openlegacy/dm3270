...
public class TerminalServer implements Runnable {
        ...
        try {
            try {
                ...
                serverSocket.connect(
                        new InetSocketAddress(serverURL, serverPort), connectionTimeoutMillis);
            ...
            while (running) {
                ...
                telnetListener.listen(
                        TelnetSocket.Source.SERVER, message, LocalDateTime.now(), true);
            }
        } catch (IOException e) {
            if (running) {
                close();
    ...
    public synchronized void write(byte[] buffer) {
        // the no-op may come here if socket is closed from remote end and client has not been
        // closed
