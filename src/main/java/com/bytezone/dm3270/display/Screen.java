...
public class Screen implements DisplayScreen {
    ...
    private static final byte[] SAVE_SCREEN_REPLY_TYPES = {
        Attribute.XA_HIGHLIGHTING,
        Attribute.XA_FGCOLOR,
        Attribute.XA_CHARSET,
        Attribute.XA_BGCOLOR,
        Attribute.XA_TRANSPARENCY
    };
    ...
    private final Set<KeyboardStatusListener> keyboardChangeListeners =
    ...
    public Screen(
        ...
        ScreenOption currentOption =
                alternateScreenDimensions == null ? ScreenOption.DEFAULT : ScreenOption.ALTERNATE;

        ScreenDimensions screenDimensions =
                currentOption.equals(ScreenOption.DEFAULT)
    ...
    public ScreenDimensions getScreenDimensions() {
        return currentScreen == ScreenOption.DEFAULT
                ? defaultScreenDimensions
                : alternateScreenDimensions;
    }
    ...
    public void eraseAllUnprotected() {
        ...
        firstUnprotectedField.ifPresent(
                screenPositions1 -> cursor.moveTo(screenPositions1.getFirstLocation()));
    }
    ...
    public void clearScreen(ScreenOption requestedScreenOption) {
        if (!requestedScreenOption.equals(currentScreen)) {
            ScreenDimensions size =
                    requestedScreenOption.equals(ScreenOption.DEFAULT)
                            ? defaultScreenDimensions
                            : alternateScreenDimensions;
    ...
    public Command readModifiedFields() {
        return screenPacker.readModifiedFields(
                currentAID, getScreenCursor().getLocation(), readModifiedAll, sscpLuData);
    }
    ...
    public AIDCommand readBuffer() {
        return screenPacker.readBuffer(
                currentAID, getScreenCursor().getLocation(), replyMode, replyTypes);
    }
